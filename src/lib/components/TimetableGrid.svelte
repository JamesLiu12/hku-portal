...
<div class="w-full overflow-x-auto">
	<div class="min-w-full border border-gray-200 rounded-lg bg-white">
		<!-- Header -->
		...
		<div class="relative" style="height: 800px;">
			<!-- Day columns -->
			<div class="grid grid-cols-8 h-full relative z-10">
				<!-- Time column -->
				...
				{#each days as day}
					<div class="relative border-r border-gray-200 last:border-r-0 h-full">
						...
						{#each getSectionsForDay(day.value) as section}
							...
							<div
								class="absolute left-0 right-0 mx-1 rounded-md p-2 text-white text-xs cursor-pointer hover:shadow-lg transition-shadow {getSectionColor(section.type)}"
								style="top: {pos.top}; height: {pos.height}; z-index: 2;"
								title="{section.courseCode} {section.section} - {section.venue} {section.building}"
							>
								<div class="font-semibold truncate">{section.courseCode}</div>
								<div class="truncate">{section.section} - {section.type}</div>
								<div class="truncate text-xs opacity-90">{section.startTime}-{section.endTime}</div>
								<div class="truncate text-xs opacity-75">{section.venue} {section.building}</div>
							</div>
						{/each}
					</div>
				{/each}
